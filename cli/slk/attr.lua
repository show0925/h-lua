--- 属性系统目标文本修正
local hslk_attr_targetLabel = function(target, actionType, actionField, isValue)
    if (actionType == 'spec'
        and isValue ~= true
...
--- 键值是否百分比数据
local hslk_attr_isPercent = function(key)
    if (table.includes({
...
--- 键值是否层级数据
local hslk_attr_isLevel = function(key)
    local a = string.find(key, "_attack")
    local p = string.find(key, "_append")
    local n = string.find(key, "e_")
...
--- 属性系统说明构成
hslk_attr_desc = function(attr, sep, indent)
    indent = indent or ""
    local str = {}
    local strTable = {}
    sep = sep or "|n"
    for _, arr in ipairs(table.obj2arr(attr, CONST_ATTR_KEYS)) do
        ...
        if (hslk_attr_isPercent(k) == true) then
            v = v .. "%"
        end
        if (hslk_attr_isLevel(k) == true) then
            v = v .. "层"
        end
        --
        if (k == "xtras") then
            ...
            for vvi, vv in ipairs(v) do
                ...
                if (CONST_EVENT_LABELS[on] ~= nil and #actions == 3) then
                    ...
                    if (odds > 0 and percent ~= nil and val ~= nil) then
                        ...
                        elseif (type(val) == 'string') then
                            ...
                            if (val == 'damage') then
                                ...
                                if (#valAttr == 2 and CONST_EVENT_TARGET_LABELS[on] and CONST_EVENT_TARGET_LABELS[on][valAttr[1]]) then
                                    ...
                                    au = hslk_attr_targetLabel(au, actionType, actionField, true)
                                    local aa = valAttr[2]
                                    if (aa == 'level') then
                        ...
                        -- 补正百分号
                        if (type(val) == 'number' and hslk_attr_isPercent(actionField) == true) then
                            valLabel = valLabel .. "%"
                        end
                        -- 对象名称修正
                        target = hslk_attr_targetLabel(target, actionType, actionField)
